...
|traceMode|`'enabled'`|Mode `'enabled'` will cause the Agent to sample as many requests as possible. Mode 'disabled' will disable sampling and metrics.|
...
|insertTraceIdsIntoLogs|`false`|Insert trace IDs into supported logging packages' output. Options are `true`, `'traced'`, `'sampledOnly'`, and `'always'`. The default, `false`, does not insert trace ids. `true` and `'traced'` insert the ID when tracing. `'sampledOnly'` inserts the ID when the trace is sampled. `'always'` inserts an empty trace ID value (all-zeroes) even when not tracing.|
...
Probe settings in the `solarwinds-apm-config` configuration file will override [probe defaults](./docs/probe-defaults.md). For example, here is how to turn off sampling for `fs`:
