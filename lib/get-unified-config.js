...
function getUnifiedConfig () {
  ...
  // only assume the use of an AppOptics collector if a custom endpoint is provided
  // and points to an AO domain
  const isAoCollector = 'endpoint' in results.global && results.global.endpoint.includes('appoptics.com')

  //
  // if an explicit trusted path has been provided, read the contents of the certificate
  // otherwise if using an ao collector, use the bundled ao certificate
  //
  if ('trustedPath' in results.global) {
    try {
      const contents = fs.readFileSync(results.global.trustedPath, 'utf8')
      results.global.certificates = contents
    } catch (e) {
      errors.push(['Cannot read certificate file', e])
    }
  } else if (isAoCollector) {
    results.global.certificates = require('./appoptics.crt')
  }
  ...
  if (!('metricFormat' in results.global)) {
    if (isAoCollector) {
      // if using an ao collector, only send TransactionResponseTime
