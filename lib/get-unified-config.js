...
function getUnifiedConfig () {
  ...
  // if no explicit trusted path has been provided also use the appoptics collector certificate
  //
  if (!('metricFormat' in results.global)) {
    if ('endpoint' in results.global && results.global.endpoint.includes('appoptics.com')) {
      ...
      if (!('certificates' in results.global)) {
        results.global.certificates = require('./appoptics.cert')
      }
    } else {
